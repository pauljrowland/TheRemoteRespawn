...
if($_POST) {
   ...
   if (isset($_POST["apikey"])){ //Was a key sent?
    ...
    elseif (($computer < 1) or ($computer > $pcsSupported)) { //ERROR: Computer out of range.
        ...
        $board = intdiv(num1: $computer - 1, num2: 8); //Take the PC number and use intdiv to work out which relay board it is attached to.
    ...
    if (empty($errorText)) { //The $errorText variable has not been set - meaning there are no errors with the syntax of the URL specified.
        $output = shell_exec(command: $piCommand); //Execute the above action chosen.
        ...
        echo $text;
        http_response_code(response_code: 202); //Set the HTTP response code to 202 (Accepted).
    }
    elseif ($authError) {
        ...
        http_response_code(response_code: 400); //Set the HTTP response code to 400 (Bad request) to signal to the sending app the URI was invalid.
        $errorText = "<h1><span class=\"red\">Failure!</span></h1>The following error was returned: <strong><span class=\"red\">$errorText</span></strong>, Please try again.<br><br>";
        echo $errorText;
    }
